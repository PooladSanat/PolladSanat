...
<script type="text/javascript">
    $(function () {
        ...
        var table = $('.data-table').DataTable({
            ...
            "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                if (aData.status == 'بدون وضعیت') {
                    $('td:eq(0)', nRow).css('color', 'rgb(255,0,0)');
                } else if (aData.status == 'در حال تولید') {
                    $('td:eq(0)', nRow).css('color', 'rgb(0,255,45)');
                }
            },
            "bInfo": false,
            "paging": false,
            ...
            ajax: "{{ route('admin.QualityControl.listt') }}",
            columns: [
        ...
        $('body').on('click', '.device_listt', function () {
            var id = $(this).data('id');
            $('#ajaxModel_devicee').modal('show');


            $("#information_dateee").DataTable().destroy();
            $('.information_dateee').DataTable({
                processing: true,
                serverSide: true,
                "searching": false,
                "lengthChange": false,
                ...
                ajax: {
                    url: "{{ route('admin.QualityControl.device.listt') }}",
                    data: {
                        device_id: id,
                    },
                },
